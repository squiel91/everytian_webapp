...
class MiniDB {
	constructor(classes_load) {
		...
		if (!('MINIDB' in localStorage)) {
			...
			for(class_name in this.classes) {
				var ids_string = localStorage[`MINIDB|${class_name}`]
				if (ids_string) {
					...
					console.log(`WARNING: Could not find ${this.classes} ids.`)
				}
			}
		}
	...
	remove(Class, options) {
	    var limbo_elements = this.get(Class, options)
	    for (var limbo_element of limbo_elements) {
	        var id = limbo_element.id;
	        var class_name = Class.name
	        delete this.collections[class_name][id];
	        localStorage[`MINIDB|${class_name}`] = JSON.stringify(Object.keys(this.collections[class_name]));
	        delete localStorage[`MINIDB|${class_name}|${id}`];
	    }
	    // I dont know what will happend so I comment it out
		// if (sync) this.sync(object);
	}
